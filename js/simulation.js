...
export function calculateFinance(state, club) {
  ...
  return {revenue:roundMoney(revenue),costs:roundMoney(costs),net:roundMoney(net),breakdown:{matchday,broadcast,sponsorship,commercial,wages:weeklyWages,operations,debtService},attendance,ticketPrice:Math.round(ticketPrice*100)/100};
}
...
export function advanceWeek(state, world) {
  ...
  s.boardConfidence=clamp(s.boardConfidence+(won?1:lost?-1:0)+cashPressure+lossPressure+wagePressure,0,100);
  ...
  if(s.week%4===0){s.inbox.unshift({id:Date.now(),type:'FINANCE',title:'Monthly financial review',text:`Revenue ${moneyShort(s.finance.history.slice(-4).reduce((a,x)=>a+x.revenue,0))}; costs ${moneyShort(s.finance.history.slice(-4).reduce((a,x)=>a+x.costs,0))}; net ${moneyShort(s.finance.history.slice(-4).reduce((a,x)=>a+x.net,0))}.`,unread:true});}
  return s;
}

function moneyShort(n){const sign=n<0?'-':'';const v=Math.abs(n);if(v>=1000000)return `${sign}£${(v/1000000).toFixed(1)}m`;if(v>=1000)return `${sign}£${Math.round(v/1000)}k`;return `${sign}£${Math.round(v)}`;}
