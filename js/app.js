...
function renderStart(){
  root.innerHTML=`<main style="min-height:100vh;display:grid;place-items:center;padding:24px"><div style="width:min(1050px,100%)"><section class="hero"><div class="hero-mark"></div><div class="hero-content"><div class="eyebrow">Ownership simulation · 2026</div><h1>The Chairman</h1><p>Own the club. Set the ambition. Control the money. Appoint the people. Live with the consequences.</p><div class="actions"><button class="btn btn-primary" data-action="newCareer">Take control</button><button class="btn" data-action="continue">Continue saved career</button></div></div></section><div class="grid grid-3" style="margin-top:18px"><div class="card card-pad"><div class="eyebrow">01</div><h3>Own the institution</h3><p class="muted">The club is bigger than the first team. Shape its infrastructure, reputation and future.</p></div><div class="card card-pad"><div class="eyebrow">02</div><h3>Run the board</h3><p class="muted">Managers, directors, finance chiefs and supporters all have expectations.</p></div><div class="card card-pad"><div class="eyebrow">03</div><h3>Make the calls</h3><p class="muted">Every major decision trades ambition against risk.</p></div></div></div></main>`;
  bind();
}
function openClubPicker(){
  const grouped=world.leagues.map(l=>`<div style="margin-bottom:22px"><div class="section-title"><h2>${l.name}</h2><span>${l.country}</span></div><div class="club-grid">${world.clubs.filter(c=>c.leagueId===l.id).map(c=>`<button class="club-card" data-club="${c.id}" style="text-align:left;color:inherit"><div class="logo">${c.name.split(' ').map(x=>x[0]).slice(0,2).join('')}</div><h3>${esc(c.name)}</h3><p>Rep ${c.reputation} · Cash ${money(c.cash)}</p></button>`).join('')}</div></div>`).join('');
  showModal(`<div class="modal-head"><div><div class="eyebrow">New ownership</div><h2 style="margin:4px 0 0">Choose your club</h2></div><button class="btn" data-close>Close</button></div><div class="modal-body">${grouped}</div>`);
...
function action(a,b){
  ...
  if(a==='stadium'){state.cash-=500000;state.news.unshift({week:state.week,title:'Stadium feasibility study commissioned',text:'The board will receive an expansion report next month.'});render();return;}
  if(a==='noop'){state.news.unshift({week:state.week,title:'No stadium capital project approved',text:'You chose to preserve cash and keep the current stadium plan unchanged.'});render();return;}
}
